...
class Job(Base):
    ...
    raw_content: Mapped[str | None] = mapped_column(Text)  # Firecrawl 抓取的原始 Markdown 或用户粘贴内容
    ...
    # pending → parsing → awaiting_confirm → awaiting_directions → researching → done / failed
    # parsing 失败时 → awaiting_manual_input（等待用户粘贴或上传截图）
